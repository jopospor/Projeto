...
Dado o usuário possui chave da API do Trello Quando houver a necessidade de acesso aos dados do Trello E fizer a chamada via GET Então o usuário não conseguirá acessar os dados do Trello, pois a chave de API por si só não concede acesso aos dados do Trello de um usuário

### **Cenário 3:** Obter token da API do Trello
Dado o usuário possua uma conta cadastrada no Trello Quando houver a necessidade de obter token da API do Trello E acessar a mesma página onde você encontrou sua chave de API ( https://trello.com/app-key) E clicar no "Token" com hiperlink na chave da API E na página aberta, clicar em  Allow Então o usuário será redirecionado para uma página que contém o token da API.
